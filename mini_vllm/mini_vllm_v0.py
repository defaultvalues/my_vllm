...
# def flashinfer_attention_forward(self, hidden_states, position_embeddings, attention_mask, **kwargs):
...
#     # ==========================================
#     # 核心修改：利用 Mask 找到所有非 Padding 的位置索引
#     # ==========================================
#     # 3. 【核心修复】鲁棒地提取有效 Token 掩码
#     if attention_mask is not None:
#         if attention_mask.dim() == 4:
#             # HF 转换后的形状: [bsz, 1, q_len, full_seq_len]
#             # 有效位置通常是 0.0，Padding 位置是极小负数 (如 -10000.0)
#             # 我们只需要判断每一行（即每个 query token）是否全部被 mask 掉了
#             # 逻辑：如果一个 query 是有效的，它至少能看到它自己，所以这一行的最大值应该是 0
#             is_valid = (attention_mask.max(dim=-1).values > -1000).view(bsz, q_len)
#         elif attention_mask.dim() == 2:
#             # 原始 2D 掩码 [bsz, q_len]
#             is_valid = (attention_mask > 0)
#         else:
#             is_valid = torch.ones((bsz, q_len), dtype=torch.bool, device=device)
#     else:
#         is_valid = torch.ones((bsz, q_len), dtype=torch.bool, device=device)

#     # 现在的 is_valid 形状一定是 [bsz, q_len]，可以安全地用于布尔索引
#     q_valid = query_states[is_valid] 
#     k_valid = key_states[is_valid]
#     v_valid = value_states[is_valid]

#     # 3. 写入 Cache
#     flashinfer.append_paged_kv_cache(
#         k_valid, v_valid,
#         inference_metadata.batch_indices,     # 这里的 indices 必须和 is_valid 对应
...
#             q_valid, global_kv_cache.kv_cache[self.layer_idx]
#         )
#     else:
...
#     # 5. 计算 o_proj (只对有效 token)
#     # out_valid shape: [Total_Valid_Tokens, Num_Heads, Head_Dim]
#     res_valid = self.o_proj(out_valid.reshape(-1, self.config.hidden_size))

#     # ==========================================
#     # 核心修改：把结果“散射”回原始形状，其他位置绝对不动
#     # ==========================================
#     # 我们预先准备一个全 0 矩阵作为增量
#     attn_output = torch.zeros((bsz, q_len, self.config.hidden_size), device=device, dtype=hidden_states.dtype)
    
#     # 使用布尔索引把结果填回去
#     attn_output[is_valid] = res_valid.to(attn_output.dtype)

#     # 最终返回给 HF 的残差连接：hidden_states + attn_output
#     # 这样 Padding 位置加的就是 0，完全保留了 hidden_states 原有的值
...
def flashinfer_attention_forward(self, hidden_states, position_embeddings, attention_mask=None, pask_key_values=None, **kwargs):
    ...
    # 6. 只对有效部分做 o_proj (性能更优)
    # out_valid shape: [Total_Valid_Tokens, Hidden_Size]
    # 1. 确保形状是 [Total_Tokens, 4096]
    # out_valid = out_valid.reshape(-1, self.config.num_attention_heads * (self.config.hidden_size // self.config.num_attention_heads))
    
    # # 2. 先进行 o_proj 计算，再放回 Padding 矩阵中
    # # 这样做效率更高，且能避免形状对不上的问题
    # res_valid = self.o_proj(out_valid) # 现在的输入是 [49, 4096]，输出也是 [49, 4096]

    # # 3. 将结果回填到 (bsz, q_len, hidden_size) 的容器中
    # output = torch.zeros((bsz, q_len, self.config.hidden_size), device=device, dtype=res_valid.dtype)
    
    # offset = 0
    # for i, req in enumerate(inference_metadata.requests):
    #     valid_len = 1 if inference_metadata.is_decode else len(req.input_ids)
    #     # 假设是 Right Padding
    #     output[i, :valid_len] = res_valid[offset : offset + valid_len]
    #     offset += valid_len
    
    # # 此时 output 已经是 [B, S, 4096]，且已经做过 o_proj 了
    # return output, None
...
def prepare_metadata(requests, kv_cache, metadata, is_decode):
    ...
    for req in requests:
        ...
        # old_seq_len = req.seq_len - (1 if is_decode else len(req.input_ids))
    ...
    while True:
        ...
        if prefill_reqs:
            ...
            for i, req in enumerate(prefill_reqs):
                attention_mask[i, :len(req.input_ids)] = 1  # 因为是 Right Padding，有效部分在左边
        ...
        if decode_reqs:
            ...
            attention_mask = torch.ones((len(decode_reqs), 1), dtype=torch.long, device=device)
