...
def preempt_request(req: Request):
    """
    被抢占时需要调用的函数，释放 KV cache 资源，清空状态，等待下一次调度
    """
    global global_kv_cache, waiting_queue
    if req.reserved_blocks > 0:
        global_kv_cache.release_reserved(req.reserved_blocks)
...
async def scheduler():
    ...
    while True:
        ...
        while waiting_queue:
            ...
            # 请求一旦进入了active_requests，就表示它已经被调度器接受了，预占了 KV cache 资源，等待被执行；如果请求还在 waiting_queue 里，就表示它还没有被调度器接受，还没有占用 KV cache 资源，继续等待被调度器接受 
            ...
            if global_kv_cache.available_blocks() >= prompt_required_blocks:  # 只要够prefill就放进来
                ...
                global_kv_cache.reserve(prompt_required_blocks)  # 预占 KV block，确保后续调度和分配时有足够的资源
                req.reserved_blocks = prompt_required_blocks # 记录预占的 KV block 数量，方便后续释放
        ...
        for req in active_requests:
            if req.stage == "PREFILL" and token_budget > 0:
                ...
                # 根据剩余的token budget决定这次prefill多少
        ...
        for req in current_batch:
            # 分配 KV block 和更新 block table 的逻辑应该放在这里，因为只有被调度到当前 batch 的请求才需要分配 KV block 和更新 block table，其他还在 waiting_queue 里的请求不需要占用 KV cache 资源
            if req.stage == "PREFILL":
                # Chunked prefill: 每次只送入一部分输入，后续根据 cursor 继续送入剩余的输入，直到整个输入都被送入模型完成 prefill 阶段
                cur_chunk = req.num_tokens_this_step  # 这次要处理的 token 数量，之前在 admission control 阶段已经根据 token budget 和 CHUNK_SIZE 确定了这个值
        ...
        prepare_metadata(current_batch, global_kv_cache, inference_metadata)
